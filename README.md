...
## 발행 — 내 목록을 상대에게 보이게 하기

관리 화면에서 맛집을 추가하거나 후보를 바꾸면 **내 브라우저에만** 저장됩니다.
상대가 링크를 열면 `index.html` 안에 담긴 목록을 보게 되므로, 내가 정리한 목록을
보여주려면 그 파일을 새로 만들어 올려야 합니다.

1. 관리 화면 아래 **📦 초대장 파일 만들기** 를 누릅니다.
2. 지금 목록이 그대로 담긴 `index.html` 이 내려받아집니다. (사진·지도 링크·고정 코스 포함)
3. 이 파일을 GitHub 저장소의 `index.html` 자리에 올리면 끝입니다.

상대가 무엇을 보게 되는지는 **👀 상대방 화면으로 미리보기** 로 먼저 확인할 수 있습니다.
상대가 코스를 고르면 그 기록(방문·밀림)이 목록에 반영됩니다. 동기화를 켜 두었다면
내 기기에도 따라옵니다. 다만 상대 기기가 서버를 읽지 못한 상태에서는 아무것도
올리지 않으므로, 낡은 목록이 내 최신 목록을 덮어쓰는 일은 없습니다.
