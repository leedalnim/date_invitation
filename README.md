...
전송 화면의 버튼으로 코스를 보냅니다. 복사되는 내용은 코스 목록과 결과 링크입니다.
링크를 열면 서버 없이도 그 코스가 그대로 보입니다.

| 여는 곳 | 동작 |
| --- | --- |
| 폰 크롬 / 사파리 | `공유하기` — 공유 시트에서 카카오톡 선택 |
| 카카오톡 인앱 브라우저 | 클립보드로 복사 → 대화창에 붙여넣기 |
| PC | 클립보드로 복사 |

카카오톡 전용 버튼은 꺼 두었습니다. 다시 켜려면 `index.html` 위쪽의 카카오 SDK
`<script>` 주석을 풀고 `KAKAO_JS_KEY` 에 JavaScript 키를 넣으세요. Kakao Developers 의
`플랫폼 > Web > 사이트 도메인` 에 배포 주소(`https://<사용자명>.github.io`)를
경로 없이 등록해야 동작합니다.
